...
Leva.Framework.Fakes           -> fake clocks, events, queues, states, routines, principal, logs, test helpers
...
Dependencies move inward toward Core. Core depends only on .NET. Engine depends on Core. Execution depends on Core. Storage depends on Core. Notifications depends on Core. Identity depends on Core. Provider libraries depend on their contract libraries and Core. Fakes depends on Core, Engine, Identity, and Notifications because it is a testing support library. Applications and future provider libraries may depend on selected framework libraries, but Core and Engine should not depend on applications, UI providers, storage providers, notification providers, identity providers, devices, databases, or web frameworks.
...
README_FAKES.md             -> Fakes and shared test helpers purpose, dependencies, overview, files and classes
README_STORAGE*.md          -> Storage contracts and provider library guides
README_NOTIFICATIONS*.md    -> Notifications contracts and provider library guides
