...
`Leva.Framework.Identity.AspNet.Tests` verifies the ASP.NET Core adapter behavior for framework identity auth sessions, claims, HTTP cookie transport, login, logout, authorization bridging, endpoint contracts, Google sign-in contracts, return URL handling, and JWT token handling.
...
`AspNetIdentityServiceTests` - Verifies login cookie writing and missing-session logout failure behavior.
...
`AspNetEndpointContractTests` - Verifies login request mapping and principal response shaping used by the built in endpoints.
`AspNetGoogleContractTests` - Verifies Google user mapping, authorization URL/correlation cookie creation, and local return URL handling.
