...
{
	[Fact]
	public void Add_CreatesLogEntryAndTraceEntry()
	{
		...
		var logEntry = runtimeLog.Add(LogCategory.Event, TraceLevel.Warning, "Something happened.", new { Count = 5 });

		Assert.Equal(logEntry, Assert.Single(runtimeLog.LogEntries));
		Assert.Equal(TraceLevel.Warning, logEntry.Level);
		Assert.Equal(5, logEntry.Properties?["Count"]);
		Assert.Equal("Something happened.", Assert.Single(traceSink.TraceEntries).Message);
	}
