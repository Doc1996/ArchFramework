...
Fakes exist for unit tests, integration-style framework tests, demos, and application tests that need predictable clocks, queues, events, traces, transitions, states, routines, behaviors, status updates, alarms, or a ready-made fake context. `Leva.Framework.Fakes` may depend on Core and Engine because it is a testing support library for those projects. Engine must never depend on Fakes. Fakes should not depend on application projects, UI providers, storage providers, notification providers, authentication providers, or external infrastructure.
...
`FakeAccess` - Simple access object for tests that exposes transition and trace capabilities.
